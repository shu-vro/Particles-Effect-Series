...
class MusicAnalyzer {
    constructor(source) {
        ...
        this.audioSource = this.audioContext.createMediaElementSource(this.source);
    ...
    getSamples() {
        ...
        let intendedArray = this.dataArray.slice(0, Math.round(this.bufferLength / 2));
...
function drawImage() {
    ...
    class Particle {
        ...
        update() {
            ...
            let distance = Math.sqrt(Math.pow(canvas.height / 2 - this.y, 2) +
            ...
            for (let x = 0; x < data.width; x++) {
                if (data.data[y * 4 * data.width + x * 4 + 3] > 128) {
                    ...
                    let color = "rgb(" +
                        ...
                        ")";
                    particlesArray.push(new Particle(positionX, positionY, color, n));
                }
            }
        }
        for (let i = 0; i < musicAnalyzer.bufferLength; i++) {
            bars.push(new Bar((canvas.width / musicAnalyzer.bufferLength) * i, 300, 0.5, 250, "white", i));
        }
    }
    ...
    function animate() {
        ...
        gradient.addColorStop(audio.currentTime / audio.duration + 0.00001, "transparent");
